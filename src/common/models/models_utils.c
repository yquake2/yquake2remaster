...
Mod_LoadAllocate(const char *mod_name, dmdx_t *dmdxheader, void **extradata)
{
	...
	dmdxheader->ofs_bones = dmdxheader->ofs_animgroup + dmdxheader->num_animgroup * sizeof(dmdxframegroup_t);
	dmdxheader->ofs_vertweights = dmdxheader->ofs_bones +  dmdxheader->num_frames * dmdxheader->num_bones * sizeof(dmdx_bone_t);
	dmdxheader->ofs_vertrefs = dmdxheader->ofs_vertweights + dmdxheader->num_vertweights * sizeof(dmdx_vertweight_t);
	dmdxheader->ofs_end = dmdxheader->ofs_vertrefs + dmdxheader->num_xyz * sizeof(dmdx_vertref_t);
...
Mod_LoadSTLookup(dmdx_t *pheader)
{
	...
	YQ2_COM_CHECK_OOM(st_lookup, "calloc()", pheader->num_st * sizeof(int))
	if (!st_lookup)
	{
		/* unaware about YQ2_ATTR_NORETURN_FUNCPTR? */
		return NULL;
