...
rotate_train_wait(edict_t *self)
{
	...
	if (self->moveinfo.wait)
	{
		...
		else if (self->spawnflags & SPAWNFLAG_ROTATE_TRAIN_TOGGLE)
		{
...
void
rotate_train_next(edict_t *self)
{
	qboolean first;
	edict_t *ent;
...
void
rotate_train_find(edict_t *self)
{
	edict_t *ent;
...
void
rotate_train_use(edict_t *self, edict_t *other, edict_t *activator)
{
	self->activator = activator;

	if (self->spawnflags & SPAWNFLAG_TRAIN_START_ON)
	{
		if (!(self->spawnflags & SPAWNFLAG_ROTATE_TRAIN_TOGGLE))
		{
			return;
		}
 ...
 *
 * Oblivion: Rotating trains are moving platforms that players can ride.
 ...
 */
void
SP_func_rotate_train(edict_t *self)
{
	if (!self)
	{
		return;
	...
	if (self->spawnflags & SPAWNFLAG_ROTATE_TRAIN_BLOCK_STOPS)
	{
		self->dmg = 0;
	}
	else if (!self->dmg)
